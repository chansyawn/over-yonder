...
- **Child ticket**: a GitHub sub-issue labelled `wayfinder:<type>` (`research`, `prototype`, `grilling`, or `task`). If sub-issues are unavailable, use a task list in the map and add `Part of #<map>` to the child.
- **Blocking**: use GitHub's native issue dependencies. If unavailable, add `Blocked by: #<n>` to the child body.
